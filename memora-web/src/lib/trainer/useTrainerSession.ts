...
export function useTrainerSession(setId: string) {
  ...
  const handleGameUpdate = useCallback((update: GameUpdate | null) => {
    ...
    // Уровень и достижения празднует общее окно игры — то же, что в курсах.
    celebrate(update);
  }, [later]);
  ...
  const finishSession = useCallback(async () => {
    ...
    setSummary(result);
    setStatus('finished');
    // Лисёнку — счёт ответов, а не карточек: одна карточка за занятие
    // встречается по нескольку раз, и «18 из 12» прозвучало бы нелепо.
    emitFox({ type: 'session_end', correct: statsRef.current.correct, total: statsRef.current.answered });
  ...
  useEffect(() => {
    ...
    // Новый вопрос на экране — у лисёнка пошёл отсчёт «думает» (снимает его ответ).
