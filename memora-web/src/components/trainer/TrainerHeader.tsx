...
export function TrainerHeader({
  position, queueLength, combo, xpFloat, toast, onSettings, onClose,
}: {
  position: number;
  queueLength: number;
  combo: number;
    ...
    <div className="sticky top-0 z-20 bg-qz-bg/95 backdrop-blur-sm" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
      ...
      {toast && (
        <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 pb-2">
          <div role="status" className="bg-qz-accent/10 border border-qz-accent/40 text-qz-accent text-sm font-semibold rounded-2xl px-4 py-2.5 animate-in fade-in slide-in-from-top-1 duration-300">
            {toast}
          </div>
        </div>
      )}
