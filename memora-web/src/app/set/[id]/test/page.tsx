...
export default function TestModePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (isConfiguring) {
        return (
            <div className="min-h-screen bg-qz-bg flex flex-col text-qz-text font-sans items-center justify-center relative">
                <button
                    onClick={closeConfig}
                    className="absolute top-6 left-6 flex items-center gap-2 text-qz-text-muted hover:text-qz-text transition-colors text-sm font-semibold"
                >
                    <X size={20} /> Назад
                </button>

                <div className="w-full max-w-2xl bg-qz-card border border-qz-border-light rounded-2xl p-8 md:p-12 shadow-2xl animate-in zoom-in duration-300">
                    <div className="flex flex-col items-center text-center mb-10">
                        ...
                        <p className="text-qz-text-muted font-medium">Модуль: {set.title}</p>
                    </div>
                    ...
                    <div className="space-y-8">
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="font-semibold text-lg flex items-center gap-2">
                                    <ListTodo size={20} className="text-qz-text-muted" />
                                    Количество вопросов
                                </h3>
                            ...
                            <input
                                ...
                                className="w-full bg-qz-bg border border-qz-border-light rounded-xl px-4 py-3 text-qz-text outline-none focus:border-indigo-500 transition-colors font-medium text-lg"
                            />
                        </div>

                        <div>
                            <h3 className="font-semibold text-lg mb-4 flex items-center gap-2">
                                <Layers size={20} className="text-qz-text-muted" />
                                Формат
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <label className="flex items-center gap-3 p-4 bg-qz-bg border border-qz-border-light rounded-xl cursor-pointer hover:border-zinc-500 transition-colors">
                                    <div className={`w-5 h-5 rounded overflow-hidden flex items-center justify-center border ${allowedTypes.trueFalse ? 'bg-[#4255ff] border-indigo-500' : 'bg-transparent border-zinc-500'}`}>
                                        {allowedTypes.trueFalse && <CheckCircle size={14} className="text-qz-text" />}
                                    </div>
                                    <input
                                        type="checkbox"
                                        className="hidden"
                                ...
                                <label className="flex items-center gap-3 p-4 bg-qz-bg border border-qz-border-light rounded-xl cursor-pointer hover:border-zinc-500 transition-colors">
                                    <div className={`w-5 h-5 rounded overflow-hidden flex items-center justify-center border ${allowedTypes.multipleChoice ? 'bg-[#4255ff] border-indigo-500' : 'bg-transparent border-zinc-500'}`}>
                                        {allowedTypes.multipleChoice && <CheckCircle size={14} className="text-qz-text" />}
                                    </div>
                                    <input
                                        type="checkbox"
                                        className="hidden"
                                ...
                                <label className="flex items-center gap-3 p-4 bg-qz-bg border border-qz-border-light rounded-xl cursor-pointer hover:border-zinc-500 transition-colors">
                                    <div className={`w-5 h-5 rounded overflow-hidden flex items-center justify-center border ${allowedTypes.written ? 'bg-[#4255ff] border-indigo-500' : 'bg-transparent border-zinc-500'}`}>
                                        {allowedTypes.written && <CheckCircle size={14} className="text-qz-text" />}
                                    </div>
                                    <input
                                        type="checkbox"
                                        className="hidden"
                                ...
                                <label className="flex items-center gap-3 p-4 bg-qz-bg border border-qz-border-light rounded-xl cursor-pointer hover:border-zinc-500 transition-colors">
                                    <div className={`w-5 h-5 rounded overflow-hidden flex items-center justify-center border ${allowedTypes.matching ? 'bg-[#4255ff] border-indigo-500' : 'bg-transparent border-zinc-500'}`}>
                                        {allowedTypes.matching && <CheckCircle size={14} className="text-qz-text" />}
                                    </div>
                                    <input
                                        type="checkbox"
                                        className="hidden"
                        ...
                        <button
                            ...
                            className="w-full bg-orange-600 hover:bg-orange-500 text-qz-text font-bold py-4 rounded-xl shadow-lg transition-all text-lg mt-8 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
    ...
    return (
        <div className="min-h-screen bg-qz-bg text-qz-text flex flex-col relative font-sans">

            {/* Top Bar - Dark Mode */}
            <header className="sticky top-0 bg-qz-bg flex justify-between items-center px-6 py-4 z-50 border-b border-qz-border-light shadow-sm">
                <div className="flex items-center gap-4">
                    <div>
                        <h1 className="text-lg font-bold text-qz-text">Тест</h1>
                        <p className="text-xs font-semibold text-qz-text-muted truncate max-w-[200px] md:max-w-md">{set.title}</p>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    <span className="text-qz-text-muted font-semibold text-sm hidden md:block">Вопросов: {testQuestions.length}</span>
                    <button onClick={() => setIsConfiguring(true)} className="p-2 hover:bg-qz-card rounded-lg transition-colors text-qz-text-muted hover:text-qz-text" title="Настройки">
                        <Settings size={24} />
                    </button>
                    <button
                        onClick={closeTest}
            ...
            <main className="flex-1 flex flex-col p-6 z-10 w-full max-w-4xl mx-auto pb-32">
                ...
                {isSubmitted ? (
                    <div className="bg-qz-card border text-center border-qz-border-light p-10 rounded-2xl w-full shadow-md animate-in zoom-in duration-500 mb-12 flex flex-col items-center">
                        ...
                        <h2 className="text-3xl font-bold mb-2 text-qz-text">Ваш результат</h2>
                        <p className="text-qz-text-muted font-medium mb-10">
                        ...
                        <div className="mt-12 flex justify-center gap-4 w-full max-w-md">
                            <button
                                ...
                                className="flex-1 items-center justify-center gap-2 bg-qz-bg border border-qz-border-light hover:border-zinc-500 text-qz-text font-bold py-3 rounded-xl transition-all"
                            >
                                Настроить
                            </button>
                            <button
                                onClick={startTest}
                                className="flex-1 items-center justify-center gap-2 bg-orange-600 hover:bg-orange-500 text-qz-text font-bold py-3 rounded-xl shadow-lg shadow-orange-600/20 transition-all"
                            >
                            ...
                            return (
                                <div key={question.flashcard.id} className="bg-qz-card border border-qz-border-light shadow-sm p-8 rounded-2xl">
                                    <div className="flex justify-between items-start mb-6 border-b border-qz-border-light pb-6">
                                        ...
                                        <span className="text-qz-text-muted font-bold text-sm bg-qz-bg px-3 py-1 rounded-full shrink-0 ml-4 mt-1 border border-qz-border-light">{qIndex + 1} / {testQuestions.length}</span>
                                    </div>
                                        ...
                                        {question.type === "WRITTEN" && question.writtenData && (
                                            <div className="col-span-1 md:col-span-2">
                                                <input
                                                    ...
                                                    className="w-full bg-qz-bg border border-qz-border-light rounded-xl px-5 py-4 focus:border-indigo-500 outline-none transition-all font-medium text-xl text-qz-text shadow-sm"
                                                    autoComplete="off"
                                                />
                                            </div>
                                        )}
                ...
                {/* Action Button */}
                {!isSubmitted && (
                    <div className="mt-12 flex justify-end sticky bottom-6 bg-qz-bg/80 backdrop-blur border border-qz-border-light p-4 rounded-2xl shadow-lg">
                        <button
                            onClick={submitTest}
                            disabled={Object.keys(userAnswers).length !== testQuestions.length || isSubmitting}
                            className="bg-orange-600 hover:bg-orange-500 disabled:bg-zinc-200 disabled:text-qz-text-muted disabled:cursor-not-allowed text-qz-text font-bold py-4 px-10 rounded-xl transition-all text-lg flex items-center justify-center gap-2 min-w-[200px]"
                        >
