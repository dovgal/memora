'use client';
// Прохождение юнита тренажёра Édito A2.
// Прогресс хранится на сервере (courseId 'edito-a2'), при полном прохождении
// юнита лексика добавляется в личный набор «Édito A2 — Словарь».
...
import { syncA2VocabSet } from '@/lib/courses/editoA2VocabSync';

export default function EditoA2UnitPage({ params }: { params: Promise<{ unitId: string }> }) {
  ...
  useEffect(() => {
    if (!idToken) return;
    let cancelled = false;
    getCourseProgress(EDITO_A2_COURSE_ID, idToken).then(entries => {
      if (cancelled) return;
  ...
  const handleExerciseComplete = useCallback((exerciseId: string) => {
    ...
    recordExerciseProgress(EDITO_A2_COURSE_ID, unitId, exerciseId, idToken);
  }, [unitId, idToken]);
  ...
  const handleMarkKnown = useCallback(async () => {
    ...
    try {
      ...
      await markUnitKnown(EDITO_A2_COURSE_ID, unitId, ids, idToken);
  ...
  // Юнит пройден полностью — синхронизируем лексику в личный набор.
  useEffect(() => {
    if (pct === 100 && totalInteractive > 0 && idToken) {
      syncA2VocabSet(idToken).catch(() => {});
    }
  ...
  if (!unit) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-qz-card text-foreground">
        ...
        <Link href="/dashboard/student/courses/edito-a2" className="text-emerald-400 hover:underline">
          ← Назад к курсу
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-qz-card text-qz-text">
      <div className="max-w-3xl mx-auto px-4 py-6 md:py-8">
        ...
        <div className="mb-6">
          <Link href="/dashboard/student/courses/edito-a2"
          ...
          <div className="bg-qz-card border border-border rounded-xl p-4 mb-6">
            ...
            {pct === 100 && (
              <p className="text-emerald-400 text-xs mt-2 flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5" /> Юнит завершён! Лексика добавлена в набор «Édito A2 — Словарь».
              </p>
            )}
          </div>
        )}
